...
class TSPEnv:
    ...
    def load_raw_data(self, episode,begin_index=0):
        ...
        if not os.path.isfile(self.data_path):
            ...
            try:
                existing = sorted(os.listdir(data_dir)) if os.path.isdir(data_dir) else []
            except OSError:
                existing = []

            hint = (
                f"Dataset file not found: {self.data_path}\n"
                f"Expected a training dataset txt under: {data_dir}\n"
                f"Existing files there: {existing}\n"
                "Please download the TSP training dataset as described in README.md and place it into TSP/data."
            )
    ...
    def drawPic(self, arr_, tour_, name='xx', optimal_tour_=None, index=None, out_dir=None):
        ...
        self.make_dir(path)
        plt.savefig(os.path.join(path, f'{name}.pdf'), bbox_inches='tight', pad_inches=0)
