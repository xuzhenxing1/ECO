...
class TSPTester():
    def __init__(self,
        ...
        # Best-case visualization (min gap instance)
        self.best_gap = float('inf')
        self.best_episode_index = None
        self.best_problem_cpu = None
        self.best_opt_tour_cpu = None
        self.best_model_tour_cpu = None
        self.best_opt_len = None
        self.best_model_len = None

    def _save_best_tour_plots(self):
        if self.best_problem_cpu is None:
            return
        idx0 = torch.tensor(0)
        n_nodes = int(self.best_problem_cpu.shape[1])
        base_name = f"best_gap_ep{self.best_episode_index}_n{n_nodes}_gap{self.best_gap:.4f}%"

        # Validate tours (should be a permutation of [0..n_nodes-1])
        try:
            opt_tour = self.best_opt_tour_cpu[0].long()
            model_tour = self.best_model_tour_cpu[0].long()

            def _tour_stats(tour_tensor):
                uniq = torch.unique(tour_tensor)
                return {
                    'len': int(tour_tensor.numel()),
                    'unique': int(uniq.numel()),
                    'min': int(tour_tensor.min().item()),
                    'max': int(tour_tensor.max().item()),
                }

            opt_stats = _tour_stats(opt_tour)
            model_stats = _tour_stats(model_tour)
            self.logger.info(
                'Best-gap instance stats: opt_len=%.6f, model_len=%.6f, gap=%.6f%%',
                float(self.best_opt_len) if self.best_opt_len is not None else float('nan'),
                float(self.best_model_len) if self.best_model_len is not None else float('nan'),
                float(self.best_gap),
            )
            self.logger.info('Best-gap opt tour stats: %s', opt_stats)
            self.logger.info('Best-gap model tour stats: %s', model_stats)
        except Exception as exc:
            self.logger.warning('Best-gap tour validation failed: %s', exc)

        # Save tour sequences for manual inspection
        try:
            figure_dir = os.path.join(self.result_folder, 'figure')
            if not os.path.exists(figure_dir):
                os.makedirs(figure_dir)
            np.savetxt(os.path.join(figure_dir, base_name + '_optimal_tour.txt'), self.best_opt_tour_cpu[0].cpu().numpy(), fmt='%d')
            np.savetxt(os.path.join(figure_dir, base_name + '_model_tour.txt'), self.best_model_tour_cpu[0].cpu().numpy(), fmt='%d')
        except Exception as exc:
            self.logger.warning('Failed to save tour txt files: %s', exc)

        self.env.drawPic(
            self.best_problem_cpu,
            self.best_opt_tour_cpu,
            name=base_name + "_optimal",
            index=idx0,
            out_dir=self.result_folder,
        )
        self.env.drawPic(
            self.best_problem_cpu,
            self.best_model_tour_cpu,
    ...
    def run(self):
        ...
        # Save plots for the best (min gap) instance
        try:
            self._save_best_tour_plots()
            if self.best_opt_len is not None and self.best_model_len is not None:
                self.logger.info(
                    "Saved best-gap tours under %s/figure (opt_len=%.4f, model_len=%.4f)",
                    self.result_folder,
                    float(self.best_opt_len),
                    float(self.best_model_len),
                )
        except Exception as exc:
            self.logger.warning('Failed to save tour plots: %s', exc)
    ...
    def _test_one_batch(self, episode, batch_size,clock=None):
        ...
        with torch.no_grad():
            ...
            # IMPORTANT: cache the full optimal tour here.
            # During RRC, env.solution is overwritten by sub-problem ranks in destroy_solution().
            optimal_tour_full = self.env.solution.detach().clone()
            ...
            # Track best instance (minimum gap) for visualization
            try:
                gap_vec = (current_best_length - self.optimal_length) / self.optimal_length * 100
                best_gap_val, best_idx = torch.min(gap_vec, dim=0)
                best_gap_val_item = float(best_gap_val.item())
                if best_gap_val_item < self.best_gap:
                    self.best_gap = best_gap_val_item
                    self.best_episode_index = int(episode + best_idx.item())
                    self.best_problem_cpu = self.origin_problem[best_idx:best_idx+1].detach().cpu()
                    # Use cached full optimal tour (not env.solution which may be a subpath rank now)
                    self.best_opt_tour_cpu = optimal_tour_full[best_idx:best_idx+1].detach().cpu().long()
                    self.best_model_tour_cpu = best_select_node_list[best_idx:best_idx+1].detach().cpu().long()
                    self.best_opt_len = float(self.optimal_length[best_idx].item())
                    self.best_model_len = float(current_best_length[best_idx].item())
                    # Save immediately so user can inspect without waiting full run
                    self._save_best_tour_plots()
            except Exception:
                pass
