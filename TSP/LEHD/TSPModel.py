...
class TSP_Decoder(nn.Module):
    ...
    def forward(self,data,selected_node_list):
        ...
        new_props = torch.zeros(batch_size_V, problem_size, device=props.device)

        index_1_ = torch.arange(batch_size_V, dtype=torch.long, device=props.device)[:, None].expand(batch_size_V, selected_node_list.shape[1])  # shape: [B*(V-1), n]
